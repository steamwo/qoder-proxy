...
async function copyEndpoint(btn){if(!currentEndpoint)return;var badge=btn.querySelector('.copy-badge');var old=badge?badge.textContent:'复制';btn.disabled=true;if(badge)badge.textContent='复制中';try{await copyText(currentEndpoint);if(badge)badge.textContent='已复制';notify('已复制端点：'+currentEndpoint,'success');}catch(e){notify('复制失败：'+(e.message||String(e)),'error-toast');}finally{setTimeout(function(){if(badge)badge.textContent=old;btn.disabled=!currentEndpoint;},700);}}
