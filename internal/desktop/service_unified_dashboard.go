  ...
  <section id="overview" class="panel active">
    <div class="card status-card">
      ...
      <div class="section">
        ...
        <div class="facts">
          <div class="fact">
            <div class="k">暴露端点</div>
            <button id="endpointCopy" class="endpoint-copy" onclick="copyEndpoint(this)" title="点击复制 URL"><span id="endpoint" class="v big endpoint">—</span><span class="copy-badge">复制</span></button>
            <div id="endpointHint" class="muted">点击复制 URL</div>
          </div>
      ...
      <div class="section">
        ...
        <div class="account-grid">
          ...
          <div class="fact"><div class="k">积分余额</div><div id="quotaRaw" class="v">—</div><div id="quotaPercent" class="muted">—</div><div class="quota-bar"><div id="quotaFill" class="quota-fill"></div></div></div>
...
function buttonBusy(btn,text){if(!btn)return function(){};var old=btn.textContent;btn.disabled=true;btn.classList.add('busy');btn.textContent=text;return function(){btn.textContent=old;btn.classList.remove('busy');btn.disabled=false;};}
...
async function refreshQuota(btn){var done=buttonBusy(btn,'刷新中…');try{var q=await jsonFetch('/admin/api/quota');document.getElementById('plan').textContent=q.plan||'Qoder';var p=Number(q.user_remaining_percent);var safeP=isFinite(p)?Math.max(0,Math.min(100,p)):NaN;document.getElementById('quotaPercent').textContent=isFinite(safeP)?(safeP.toFixed(1)+'% 可用'):'—';document.getElementById('quotaFill').style.width=(isFinite(safeP)?safeP:0)+'%';document.getElementById('quotaReset').textContent=fmtDate(q.reset_at);var limit=Number(q.user_limit);var remaining=Number(q.user_remaining);if(limit>0&&isFinite(safeP)){remaining=limit*safeP/100;}document.getElementById('quotaRaw').textContent=(limit>0)?(fmtNum(remaining)+' / '+fmtNum(limit)):'—';document.getElementById('quotaFetched').textContent=q.fetched_at?('更新 '+fmtDate(q.fetched_at)):'—';if(btn)notify('额度已刷新','success');}catch(e){resetQuota('额度获取失败');document.getElementById('quotaFetched').textContent=e.message||String(e);if(btn)notify('额度刷新失败：'+(e.message||String(e)),'error-toast');}finally{done();}}
...
async function logout(btn){if(!confirm('退出 Qoder 账号？当前代理会停止，并删除本机保存的 Qoder 凭据。'))return;var done=buttonBusy(btn,'退出中…');busy=true;try{await jsonFetch('/admin/api/logout',{method:'POST'});document.getElementById('authHint').textContent='账号已退出';await refresh();resetQuota('未授权');notify('账号已退出，代理已停止','success');}catch(e){err('overviewError',e.message||String(e));notify('退出账号失败：'+(e.message||String(e)),'error-toast');}finally{busy=false;done();}}
